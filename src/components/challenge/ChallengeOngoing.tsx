    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-6 py-6">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-3">
            {challenge.participants.map((participant) => (
              <div key={participant.userId} className="bg-white border border-gray-200 rounded-xl p-4">
                ...
                <div className="flex justify-between text-sm text-gray-600">
                  <span>진행 시간: {formatTime(participant.current_time_minutes)}</span>
                  <span>달성률: {participant.achievement_rate.toFixed(1)}%</span>
                </div>
                <div className="mt-2 bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                    style={{ width: `${Math.min(participant.achievement_rate, 100)}%` }}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="mt-auto">
          <button className="w-full bg-blue-500 text-white py-4 rounded-2xl font-semibold text-lg hover:bg-blue-600 transition-colors">
            챌린지 상세 보기
          </button>
        </div>
      </div>
