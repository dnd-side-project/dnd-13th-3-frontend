...
export default function MissionSelectButton({ selectedMission, isModalOpen, onClick }: MissionSelectButtonProps) {
  return (
    <button
      type='button'
      onClick={onClick}
    ...
    >
      ...
      <div className='w-4 h-4 relative'>
        <img 
          ...
          alt="화살표" 
          className={`w-4 h-4 transition-transform duration-300 ${isModalOpen ? 'rotate-180' : ''}`}
        />
      </div>
    </button>
  );
